//#ifndef _FEATURE_RADDCT_
#pragma once
#define _FEATURE_RADDCT_
...
// test
class feature_radDCT : public iFeature {
	// C&D
public:
	virtual ~feature_radDCT() {}

	// ACC
public:
	virtual Mat extract(Mat input) {
		return extract(input, 20, 20);
	}
	virtual Mat extract(Mat input, int from, int to) {
		...
		//namedWindow("rad", cv::WINDOW_NORMAL);	
		//namedWindow("dct", cv::WINDOW_NORMAL);	
		//imshow("in", input);	
		//waitKey(0);	
		...
		//imshow("dct", rad);	
		//waitKey(0);	
		rad=rad(Rect(0, 0, from, to)).clone();
