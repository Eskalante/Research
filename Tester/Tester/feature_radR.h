//#ifndef _FEATURE_RADR_
#pragma once
#define _FEATURE_RADR_
...
// test
class feature_radR : public iFeature {
	// C&D
public:
	virtual ~feature_radR() {}

	// ACC
public:
	virtual Mat extract(Mat input) {
		return extract(input, 135, 150);
	}
	virtual Mat extract(Mat input, int from, int to) {
		...
		radd=rotate(rad, ((from/150.0)*360.0));
		...
		*/
		...
		//cv::reduce(radd, radd, 0, REDUCE_AVG);
